...
/**
 * A stream for writing a file into JSS. The data will be persisted to a temporary directory on the
 * local disk and copied as a complete file when the {@link #close()} method is called.
 */
@NotThreadSafe
public class JSSOutputStream extends OutputStream {
  private static final Logger LOG = LoggerFactory.getLogger(JSSOutputStream.class);
  ...
  /**
   * The JSS client for JSS operations.
   */
  ...
  /**
   * Creates a name instance of {@link JSSOutputStream}.
   *
   ...
   */
  public JSSOutputStream(String bucketName, String key, JingdongStorageService client)
      throws IOException {
    ...
    Preconditions.checkArgument(key != null && !key.isEmpty(),
        "JSS path must not be null or empty.");
    Preconditions.checkArgument(client != null, "JSSClient must not be null.");
   ...
   * uploaded to JSS Service. Once the file is uploaded, the temporary file is deleted.
   *
   * @throws IOException if an I/O error occurs
   */
  @Override
  public void close() throws IOException {
    ...
    try {
      mClient.bucket(mBucketName).object(mKey).entity(mFile).put();
      mFile.delete();
    } catch (Exception e) {
